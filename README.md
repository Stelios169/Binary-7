# <img src="Logo/URL_Logo.png" alt="Logo EcoDine" style="width: 150px; height: 150px; border-radius: 50%;"> EcoDine
...
1. Κλωνοποίηση του repository από το GitHub
Άνοιξε το Cygwin και εκτέλεσε την εντολή για να κλωνοποιήσεις το repository:

   ```bash
   git clone https://github.com/Stelios169/Binary-7.git
   cd Binary-7
   ```
2. Εγκατάσταση των απαιτούμενων εργαλείων
Αν δεν έχεις ήδη εγκαταστήσει Maven, κάνε το με την εξής εντολή:

```bash
apt-cyg install maven
```
Αν δεν έχεις εγκαταστήσει το Git στο Cygwin, κάνε το με την εντολή:

```bash
apt-cyg install git
```
3. Εκτέλεση του Maven Build
Αφού πληκτρολογήσεις και τρέξεις την εντολή:

```bash
cd demo
```
Στον κατάλογο του repository, χρησιμοποίησε την εντολή Maven για να κάνεις build το project:

```bash
mvn clean install
```
Αυτή η εντολή θα κατεβάσει τις εξαρτήσεις του project και θα το κατασκευάσει. Αν υπάρχει κάποιο σφάλμα κατά τη διάρκεια της διαδικασίας, θα το δεις στην έξοδο του Maven.
---

## Οδηγίες Εκτέλεσης

1. Εκκίνηση της Εφαρμογής (Spring Boot)
Το project φαίνεται είναι Spring Boot εφαρμογή, οπότε μπορείς να το εκκινήσεις με την εξής εντολή:

```bash
mvn spring-boot:run
```
Αυτό θα ξεκινήσει την εφαρμογή.

2. Επισκόπηση της Εφαρμογής
Αν η εφαρμογή τρέξει επιτυχώς, μπορείς να την ανοίξεις στο πρόγραμμα περιήγησης στο http://localhost:8080.
---
