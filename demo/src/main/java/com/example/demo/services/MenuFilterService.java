 ...
 */
 ...
 public class MenuFilterService {
     ...
     private double calculateProfit(Dish dish) {
         double ingredientCost = dish.getIngredients().stream()
             .mapToDouble(dishIngredient -> {
                 ...
                 return ingredient.getIngredient_cost() * ingredientQuantity;
             })
             .sum();
     ...
     // Φιλτράρει τα πιάτα με βάση τις κατηγορίες
     public List<Dish> filterByCategory(String[] categories) {
         return dishRepository.findAll().stream()
                 .filter(dish -> containsCategory(dish, categories))
                 .collect(Collectors.toList());
     }
  
     // Φιλτράρει τα πιάτα που δεν περιέχουν συγκεκριμένα αλλεργιογόνα
     public List<Dish> filterByAllergens(String[] allergies) {
         return dishRepository.findAll().stream()
