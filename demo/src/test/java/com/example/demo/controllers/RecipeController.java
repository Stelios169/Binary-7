   ...
   @Test
    ...
    @Test
        void testGetOptimizedRecipes() {
            ...
            when(mockService.findOptimizedRecipes(LocalDate.now().plusDays(7), budget)).thenReturn(mockRecipes);

            // Call the method
            String viewName = controller.getOptimizedRecipes(budget, mockModel);

            // Verify interactions
            verify(mockService).findOptimizedRecipes(LocalDate.now().plusDays(7), budget);
